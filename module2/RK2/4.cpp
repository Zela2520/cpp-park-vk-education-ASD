...
void Tree<T, IsLess>::Add(const T& elem) {
    ...
    while (it) {
        saved_parent = it;
        if (!isLess(*new_node, *it) && !isLess(*it, *new_node)) { // ==
            return;
        }
    ...
    if (del_node->right_child) {
        ...
        if (!del_node->left_child) {
            ...
            if (del_node == root) {
                root->value = 0;
                root->size--;
                // cout << root->size << " " << root->height << endl;
                return;
            }
...
int main() {
    ...
    while (cin >> operation >> value) {
        ...
        case 'e':
            ...
            cout << (tree->Find(value) ? "true" : "false") << endl;
            break;
        case 'd':
        ...
        case 'n':
            ...
            more = value;
            tree->postOrderTree(tree->Root(), [&more, &value](const Node<int>& it) {
                if (it.value > value) {
                    if (more == value) {
                        more = it.value;
                    } else if ((it.value - more) < 0) {
                        more = it.value;
                    }
                }
            });
            ...
            break;
        case 'p':
            assert(operation == "prev");
            less = value;
            tree->postOrderTree(tree->Root(), [&less, &value](const Node<int>& it) {
                if (it.value < value) {
                    if (less == value) {
                        less = it.value;
                    } else if ((it.value - less) > 0) {
                        less = it.value;
                    }
                }
            });
