// Поиск кратчайшего пути до листа
// Решение базируется на preOrderTree где мы натыкаясь на лист ищем его высоту.

// Одинаковое значение во всех узлах бинарного дерева
// Пройти по дереву любым обходом сравнивая со значением в корне.
...
struct Node {
    ...
    Node() : parent(nullptr), left_child(nullptr), right_child(nullptr), height(1), size(1) {}
    Node(T _value) : parent(nullptr), left_child(nullptr), right_child(nullptr), height(1), size(1), value(_value) {}
};


template<typename T, class IsLess = IsLessDefault<T>>
class Tree {
    Node<T>* root;
...
// 11 7 16 19 14 17 21 3 9 4 8 12 13 18 23 3 8
// 5

// 11 7 16 19 14 17 21 3 9 4 8 12 13 18 23 3
// 4

int main() {
    auto t = std::make_unique<Tree<int>>();
    ...
    int min_heigh = -1;
    t->preOrderTree(t->Root(), [&min_heigh, root = t->Root()](const Node<int>& it) {
        if (!it.left_child && !it.right_child) {
            auto tmp = &it;
            int heigh = 1;
            while (tmp != root) {
                ++heigh;
            ...
            min_heigh = (heigh < min_heigh) ? heigh : min_heigh;
        }
        ;});

    std::cout << min_heigh << std::endl;
    return 0;
}
